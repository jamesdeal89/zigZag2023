...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            if self._CurrentPlayer.ChoiceIsRaaket(Choice) and self._CurrentPlayer.GetRaaketStatus() is False:
                # call the Player's raaket calculator
                # pass in the board and the raaket origin
                self._CurrentPlayer._CalculateRaaketMove(self._Board,StartSquareReference,self._CurrentPlayer.GetDirection())
                self._CurrentPlayer.SetRaaketUsed()
            # task 9
            # only ask for finish square reference if the choice is NOT a raaket
            if not self._CurrentPlayer.ChoiceIsRaaket(Choice):
                while not SquareIsValid:
    ...
    def __CreateMoveOptionOffer(self):
        # task 9
        self._MoveOptionOffer.append("raaket")
    ...
    # task 9
    def __CreateRaaketMoveOption(self, Direction):
        NewMoveOption = MoveOption("raaket")
        NewMove = Move(0,0)
    ...
    def __CreateMoveOption(self, Name, Direction):
        ...
        # task 9
            ...
            return self.__CreateRaaketMoveOption(Direction)
        else:
...
class Player:
    def __init__(self, N, D):
        ...
        # task 9
        self.__RaaketUsed = False

    # task 9
    def _CalculateRaaketMove(self,Board,StartSquareReference,Direction):
        columnStart = int(str(StartSquareReference)[1])

        # loop from where the raaket was shot, to end of board
        # use direction as step to decide which end of board to move to,
        if Direction == -1:
            endRange = 0
            rowStart = int(str(StartSquareReference)[0])-1
        elif Direction == 1:
            # range end is NOT inclusive and we want to end at row index 5 counting from 0
            endRange = 6
            rowStart = int(str(StartSquareReference)[0])+1
        for row in range(rowStart,endRange,Direction):
            if Board[self.calculateIndex(int(str(row)+str(columnStart)))].GetPieceInSquare() is not None:
                # unless inside a kotla
                if Board[self.calculateIndex(int(str(row)+str(columnStart)))].ContainsKotla() is False:
                    # give points for the piece
                    self.ChangeScore(Board[self.calculateIndex(int(str(row)+str(columnStart)))].GetPieceInSquare().GetPointsIfCaptured())
                    # destroy the piece
                    Board[self.calculateIndex(int(str(row)+str(columnStart)))].RemovePiece()


    def calculateIndex(self, SquareReference):
        Row = SquareReference // 10
        Col = SquareReference % 10
        return (Row - 1) * 6 + (Col - 1)

    # task 9
    def GetRaaketStatus(self):
        return self.__RaaketUsed

    # task 9
    def SetRaaketUsed(self):
        self.__RaaketUsed = True
    ...
    # task 9
    def ChoiceIsRaaket(self, choice):
        MoveOption = self.__Queue.GetMoveOptionInPosition(choice-1)
        if MoveOption.GetName() == "raaket":
            return True

    def CheckPlayerMove(self, Pos, StartSquareReference, FinishSquareReference):
        Temp = self.__Queue.GetMoveOptionInPosition(Pos - 1)
        # task 9
        if Temp.GetName() == "raaket":
            # if the move is the raaket, don't check the finish square reference
            return True
